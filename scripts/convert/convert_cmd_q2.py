...
def parse_args():
    ...
    parser.add_argument('-d', '--data_type', type=str, required=True, choices=['functions', 'species'], help='Provide type of data - functional or taxonomical')
    parser.add_argument('--chunksize', type=int, default=2000, help='Number of rows to process at a time')
    parser.add_argument('--strat', action='store_true', help='If flag, output is stratified')
    return parser.parse_args()

def process_name(index_name, data_type, strat):
    if data_type == 'species':
        species_match = re.search(r's__([^|;]+)', index_name)
        processed_name = species_match.group(1) if species_match else 'unclassified'
        ...
        if strat:
            if '|' not in index_name:
                return
            processed_name = index_name.replace(' ', '_')
        else:
...
def write_to_matrix(input_path, output_path, data_type, chunksize, strat):
    ...
    accumulated_df = None
    indexcol_name = 'species' if data_type == 'species' else 'Function'
    
    for chunk in tqdm(reader, desc="Processing chunks"):
        # Filter out UNMAPPED and UNINTEGRATED
        ...
        for idx in chunk.index:
            processed_name = process_name(idx, data_type, strat)
        ...
        if processed_rows:
            ...
            # Group by index (species/function) and sum
